...
public class BorderCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (args.length == 0) {
            handleHelp(sender); // Show help if no command is typed
            return true;
        }
    ...
    private void handleSet(CommandSender sender, String[] args) {
        ...
        if (args.length < 3 || args.length > 4) {
            sender.sendMessage(ChatColor.RED + "Usage: /border set <world> <size> [time]");
            return;
        }
        ...
        try {
            double newSize = Double.parseDouble(args[2]);
            long seconds;

            if (args.length == 4) {
                seconds = parseTime(args[3]);
            } else {
    ...
    private void handleHelp(CommandSender sender) {
        // THIS IS THE NEW PERSONALIZED PART!
        String name = sender.getName(); // Gets the player's name

        if (sender.isOp()) {
            sender.sendMessage(ChatColor.GOLD + "--- BorderControl Admin Help for " + name + " ---");
            sender.sendMessage(ChatColor.YELLOW + "/border set <world> <size> [time] - Sets a world's border size.");
            ...
            sender.sendMessage(ChatColor.GOLD + "--- BorderControl Help for " + name + " ---");
            sender.sendMessage(ChatColor.YELLOW + "/border see - Shows the current border size.");
            sender.sendMessage(ChatColor.YELLOW + "/border help - Shows this help message.");
        }
    }
